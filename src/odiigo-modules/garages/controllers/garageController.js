...
exports.getGarages = asyncHandler(async (req, res) => {
  ...
  if (city) filter["location.city"] = new RegExp(city, "i");
  if (vehicleBrand) {
    // Search for garages with the specified vehicle brand
    filter.vehicleBrands = { $in: [vehicleBrand] };
  }
  ...
  try {
    ...
    const garages = await Garage.find(filter)
      ...
      .populate("services", "service_name")
      .populate("vehicleBrands", "name")
      .populate("vehicleModels", "name brand");
...
// @access  Public
exports.createGarage = asyncHandler(async (req, res) => {
  const { name, ownerDetails, location, services, vehicleBrands, vehicleModels } = req.body;
  ...
  // Fetch the created garage with populated fields for the response
  const populatedGarage = await Garage.findById(createdGarage._id)
    .populate("services", "service_name")
    .populate("vehicleBrands", "name")
    .populate({
      path: "vehicleModels",
  ...
  res.status(201).json({ success: true, data: populatedGarage });
});
...
exports.updateGarage = asyncHandler(async (req, res) => {
  ...
  if (!name || !ownerDetails || !location) {
    return res.status(400).json({
      success: false,
      message: "All required fields must be provided",
      requiredFields: ["name", "ownerDetails", "location"],
    });
  }
